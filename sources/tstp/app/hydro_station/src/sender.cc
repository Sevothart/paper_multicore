...
int Sender::trySendingAndStore(){
    ...
    x<<"Try sending....\n";
    char buf[100];
    x<<"Buffer created, calling mHandler->build\n";
    mHandler->build(buf);
    x<<"Building: "<<buf<<"\n";
    ...
    if(!send) {
        ...
        unsentMessages++;
        x<<"Unsent messages: "<<unsentMessages<<"\n";
    }
...
int Sender::getSignalStrength(){    
    return gprs->get_signal_quality();    
}
...
bool Sender::__initNetwork(){	
    ...
    while(!res) {
        ...
        res = gprs->await_response("OK", EMOTEGPTMSHORTDELAY);
