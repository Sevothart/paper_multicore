...
class Semaphore_PIP: protected Semaphore_RT
{
public:
    Semaphore_PIP(int value = 1):Semaphore_RT(value) {
    }
...
class Semaphore_MPCP_Global : public Semaphore_IPCP
{
    Semaphore_MPCP_Global(Priority_t ceiling):Semaphore_IPCP(toGlobalCeiling(ceiling)) {
    }
    
    Priority_t toGlobalCeiling(Priority_t ceiling)
    {
