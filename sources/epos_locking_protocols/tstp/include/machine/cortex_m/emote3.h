...
// UART
    void uart_init(unsigned int unit)
    {
        ...
        if(unit == 0) {
            ...
            ioc(UARTRXD_UART0) = (0 << 3) + 2;

            //5. Set GPIO pins A1 and A0 to peripheral mode
            gpioa(AFSEL) |= (PIN2) + (PIN1);
        }
        else
        {
    ...
    static void config_SSI(volatile Log_Addr * base, Reg32 clock, SSI_Frame_Format protocol, SSI_Mode mode, Reg32 bit_rate, Reg32 data_width) {
        ...
        if(base == reinterpret_cast<Log_Addr *>(SSI0_BASE)) {
            ...
            if(mode == MASTER)
                ...
                ioc(CLK_SSI_SSI0) = 0x00; //PA2

            ioc(PD4_SEL) = SSI0_FSS_OUT;
            ioc(PC6_SEL) = SSI0_TXD;
            ...
            if(mode == MASTER)
                ioc(PA0_SEL) = SSI1_CLK_OUT;
            else
                ioc(CLK_SSI_SSI1) = 0x00; //PA0

            ioc(PD4_SEL) = SSI1_FSS_OUT;
            ioc(PB5_SEL) = SSI1_TXD;
            ioc(SSIRXD_SSI1) = 0x17; //PC7
        }
        ...
        //Set GPIO pins A0(clk), B5(txd), C7(rxd), D4(fss) to peripheral mode
        gpioa(AFSEL) |= (PIN0);
        gpioc(AFSEL) |= (PIN6);
        gpioc(AFSEL) |= (PIN7);
        gpiod(AFSEL) |= (PIN4);
