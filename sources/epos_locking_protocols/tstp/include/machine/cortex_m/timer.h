...
public:
    Cortex_M_GPTM(int channel, const Count & count, bool interrupt = true, bool periodic = true, bool count_up = false)
    : _channel(channel), _base(reinterpret_cast<Reg32 *>(TIMER0_BASE + 0x1000 * channel)) {
        disable();
        power(FULL);
        reg(GPTMCFG) = 0; // 32-bit timer
        reg(GPTMTAMR) = (periodic ? 2 : 1) | (count_up? 0x10 : 0x00); // 2 -> Periodic, 1 -> One-shot
        reg(GPTMTAILR) = count;
        enable();
    }
...
public:
    ...
    void load(Count c) { reg(GPTMTAILR) = c; }
